...
def analyze_cn_topk(dev, st_file_path, cui_file_path, input_file_path):
    ...
    for index, [pre_cuis, input] in enumerate(zip(dev_pre_cui, dev_input)):
        ...
        if dev_pre_st[index][0] in st_pre:
            pre_cui = pre_cuis[st_pre.index(dev_pre_st[index][0])]
        else:
            pre_cui = pre_cuis[0]
        if cui == pre_cui:
            count += 1
        ...
        #     for idx, cui_pre_single in enumerate(pre_cuis[:2]):
        #         print(cui_pre_single, dev_pre_score_cui[index][idx],
        #               st_pre[idx], dev_pre_st[index][idx])
        #     if "CUI-less" not in st_pre[:2] and dev_pre_score_cui[index][
        #             0] - dev_pre_score_cui[index][1] <= 0.05:
        #         pre_cui = pre_cuis[st_pre[:2].index(dev_pre_st[index][0])]
        #     else:
        #         pre_cui = pre_cuis[0]
    ...
    #     #             #     print()
    #     #             #     print()
    print("acc", count / count_all)#, count_st / count_all,
          #count_st_a / count_all)  #, "ambigupus", countst / countnot,
    #countst / count_all)
