...
from transformers.integrations import (  # isort: split
    ...
    run_hp_search_optuna, run_hp_search_ray, init_deepspeed,
)
...
from transformers.file_utils import WEIGHTS_NAME, is_apex_available, is_datasets_available, is_in_notebook, \
    is_sagemaker_distributed_available, is_torch_tpu_available, is_training_run_on_sagemaker
...
from transformers.trainer_pt_utils import DistributedLengthGroupedSampler, DistributedSamplerWithLoop, \
    DistributedTensorGatherer, LabelSmoother, LengthGroupedSampler, SequentialDistributedSampler, \
    distributed_broadcast_scalars, distributed_concat, get_parameter_names, nested_concat, nested_detach, \
    nested_numpify, nested_xla_mesh_reduce, reissue_pt_warnings
from transformers.trainer_utils import PREFIX_CHECKPOINT_DIR, BestRun, EvalPrediction, HPSearchBackend, \
    PredictionOutput, ShardedDDPOption, TrainerMemoryTracker, TrainOutput, default_compute_objective, \
    default_hp_space, denumpify_detensorize, get_last_checkpoint, set_seed, speed_metrics
...
class Trainer:
    ...
    def __init__(
        ...
        if (self.is_model_parallel or (args.deepspeed and args.do_train)
                or (args.fp16_full_eval and not args.do_train) or
            (self.sharded_ddp
        ...
        default_label_names = (
            ["start_positions", "end_positions"] if type(self.model).__name__
            in MODEL_FOR_QUESTION_ANSWERING_MAPPING_NAMES.values() else
            ["labels"])
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer is None:
            decay_parameters = get_parameter_names(self.model,
                                                   [torch.nn.LayerNorm])
            decay_parameters = [
    ...
    def _report_to_hp_search(self, trial: Union["optuna.Trial",
                                                Dict[str, Any]], epoch: int,
    ...
    def save_model(self, output_dir: Optional[str] = None):
        ...
        elif (ShardedDDPOption.ZERO_DP_2 in self.args.sharded_ddp
    ...
    def predict(self,
        ...
        output = self.prediction_loop(test_dataloader,
